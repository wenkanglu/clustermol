...
def export_dendrogram(hierarchical_type, linkage, visualise_option, destination):
    ...
    plot.savefig(os.path.join("data", "data_dest", destination, "dendrogram-clustering-%s.png" % hierarchical_type))
    if visualise_option:
        plot.show()


# Method used to run specific type of hierarchical clustering, based on users choices.
def clustering(hierarchical_type, rmsd_matrix_temp, visualise_option, destination):
...
def runClustering(filename, destination, type, visualise):
    rmsd_matrix_temp = preprocessing(os.path.join("data", "data_src", filename))
    clustering(type, rmsd_matrix_temp, visualise, destination)
