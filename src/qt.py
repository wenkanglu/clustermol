...
def qt_orginal(rmsd_matrix, cutoff, minimum_membership):
    ...
    clusters_arr = numpy.ndarray(len(rmsd_matrix), dtype=numpy.int64) # Frame size needs to change
    clusters_arr.fill(-1)
    ...
    while True:
        ...
        if (degrees == 0).all():
            break
    scatterplot(clusters_arr, len(rmsd_matrix), "QT_original")
...
def qt_like(rmsd_matrix, cutoff, minimum_membership):
    ...
    centers = []
    cluster = 0
    labels = numpy.empty(len(rmsd_matrix))
    ...
    while cutoff_mask.any():
        ...
        cutoff_mask[:,members] = False
        cluster = cluster + 1
    scatterplot(labels, len(rmsd_matrix), "QT_like")
