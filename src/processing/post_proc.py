...
def save_largest_clusters(n, traj, labels, dest, type):
    ...
    for l in labels:
        ...
        il += 1
    for k in n_labels:
        trajectories[i].save_pdb(DATA + DATA_DEST + dest + "/%s-cluster%d" % (type, k) + ".pdb")
...
def embedding_plot(cluster_labels, data, dest, type, preprocess):
    ...
    plot.scatter(data[clustered, 0],
                 ...
                 label='Clusters')
    plot.legend()
    plot.savefig(DATA + DATA_DEST + dest + '/' + preprocess + '-embedding-%s.png' % type)
    plot.show()
